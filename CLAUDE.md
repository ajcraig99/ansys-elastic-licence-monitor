...
**Configuration model.** Site-specific values live in `config.json` next to `common.ps1`. `Import-AppConfig` (called at dot-source time) overrides the empty defaults from the file when present. Out-of-the-box detection works without any config; the perpetual-context enrichment in toast 1 (the "perpetual is held by [user]" variant) requires `licenseServer.host`/`port` and `perpetualFeatures` to be set. The installer copies `config.json` with `onlyifdoesntexist` so admin edits survive in-place reinstalls.
